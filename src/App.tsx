...
const DEFAULT_DATA: MeetingData = {
  ...
  videoReason: "고성과 조직의 핵심 요소를 함께 고민하고 조직에 적용하는 방향성에 대한 내용입니다.",
  ...
  presentations: [
    {
      ...
      discussion: [
        {
          agenda: "안드로이드 및 PC/POS 원터치 설치 현장 전파",
          relatedDept: "영업, 현업, 매장관리팀",
          keyContent: "- 로그인 화면 '앱 설치하기' 버튼 추가로 점주 초기 설치 유도\n- 설치 완료 시 버튼 자동 숨김 처리로 혼선 방지\n- PC 및 POS 단말기에서도 독립창 실행 가능 장점 홍보",
          discussionPoints: "- 영업 사원의 설치 프로세스 숙지 및 사전 교육 필요\n- 매장 내 다양한 기기에서의 프로그램 실행 최적화 점검",
          risks: "- 설치 과정 중 점주의 혼란 가능성\n- 교육 미흡 시 현장 혼선 및 문의 증가 위험",
        },
        {
          agenda: "아이폰(iOS) 사용 점주 추가 안내 가이드 수립",
          relatedDept: "영업 스텝, IT지원팀",
          keyContent: "- iOS의 엄격한 정책으로 수동 설치 프로세스 공유\n- 아이폰 사용자 전용 '홈 화면 추가' 안내 매뉴얼 제작 및 영업 스텝 전달",
          discussionPoints: "- 아이폰 사용자 비중 파악 및 별도 안내 체계 구축\n- 영업 스텝 숙련도 강화 및 지속 모니터링",
          risks: "- iOS 정책 변경 시 추가 대응 필요성 상존\n- 안내 부재 시 사용자 불편 및 설치율 저하 우려",
        },
        {
          agenda: "카카오톡 링크 접속 시 브라우저 전환 대응",
          relatedDept: "영업, CS팀, 마케팅",
          keyContent: "- 카톡 링크 접속 시 홈 화면 바로가기 생성 불가 문제 정의\n- 일반 브라우저 전환 자동 안내 팝업 적용 및 현장 가독성 검토\n- 접속 문의 대응 방안 수립",
          discussionPoints: "- 자동 안내문구 효과성 및 이해도 확인\n- 영업 및 CS팀 간 문의 응대 체계 강화",
          risks: "- 안내 팝업 부적절 시 점주의 혼란 및 불만 가능성\n- 고객 문의 폭증 대비 리소스 확보 필요",
        },
        {
          agenda: "현장 테스트(의견 수렴) 피드백 창구 일원화",
          relatedDept: "영업, 현업, 정보전략팀",
          keyContent: "- 6월 16일 라이브 오픈 전 테스트 기간 관리\n- 화면 로딩 속도 및 AI 디자인 아이콘 직관성 현장 피드백 수집\n- 영업 스텝과 IT팀 간 소통 채널 신설",
          discussionPoints: "- 체계적 피드백 수집 및 반영 프로세스 마련\n- 부서 간 소통 활성화 및 빠른 이슈 해결",
          risks: "- 피드백 누락 및 미반영 시 사용자 불만 증가\n- 의사소통 미비로 문제 해결 지연 가능성",
        },
      ],
    },
    {
    ...
    hasDetail: true,
      script: (
        ...
        {p.team === "정보전략팀"
          ...
          : <p className="text-gray-500 pl-3">(발표 후) 질문있으신 분들께서는 질문해 주시기 바랍니다.</p>
        }
        {p.topic && (
...
export default function App() {
  ...
  const [data, setData] = useState<MeetingData>(() => {
    try {
      const saved = localStorage.getItem("meeting_data_v11");
